/*
 * Ported from @react-stately/color (Adobe, Apache License 2.0).
 * Implements the same `Color` API surface used by the ColorField / ColorPicker family.
 */

export type ColorChannel =
	'red' | 'green' | 'blue' | 'hue' | 'saturation' | 'lightness' | 'brightness' | 'alpha';
...
export function snapValueToStep(
	...
	if (!isNaN(minValue)) {
		if (snappedValue < minValue) {
			...
			snappedValue = minValue + Math.floor((maxValue - minValue) / step) * step;
		}
	} else if (!isNaN(maxValue) && snappedValue > maxValue) {
		snappedValue = Math.floor(maxValue / step) * step;
	}
...
// English strings from @react-stately/color intl messages.
...
export abstract class Color {
	...
	getChannelValue(channel: ColorChannel): number {
		if (channel in this) {
			return (this as unknown as Record<ColorChannel, number>)[channel];
		}
	...
	withChannelValue(channel: ColorChannel, value: number): Color {
		if (channel in this) {
			const x = this.clone();
			(x as unknown as Record<ColorChannel, number>)[channel] = value;
			return x;
		}
	...
	// The locale parameter is kept for API compatibility, but only English strings are supported.
	// eslint-disable-next-line @typescript-eslint/no-unused-vars -- locale は API 互換のため受けるが英語のみ対応
	...
	getColorName(locale = 'en-US'): string {
		...
		const [hue, adjustedLightness] = this.getOklchHue(l, c, h);
		l = adjustedLightness;
		...
		if (alpha < 1) {
			...
			return `${lightness} ${chroma} ${hue}, ${percentTransparent} transparent`
				.replace(/\s+/g, ' ')
				.trim();
		} else {
			return `${lightness} ${chroma} ${hue}`.replace(/\s+/g, ' ').trim();
		}
	}

	private getOklchHue(l: number, c: number, h: number): [string, number] {
		if (c < GRAY_THRESHOLD) {
			return ['gray', l];
		}

		for (let i = 0; i < OKLCH_HUES.length; i++) {
			const [hue, initialHueName] = OKLCH_HUES[i];
			let hueName = initialHueName;
	...
	getHueName(_locale?: string): string {
		...
		const [name] = this.getOklchHue(l, c, h);
		return name;
	}
}
...
class RGBColor extends Color {
	...
	formatChannelValue(channel: ColorChannel, locale = 'en-US'): string {
