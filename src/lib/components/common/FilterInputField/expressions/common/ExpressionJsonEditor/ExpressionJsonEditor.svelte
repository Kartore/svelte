...
<div class="flex min-h-0 flex-1 flex-col gap-2">
	<div class="flex min-h-72 flex-1 overflow-hidden rounded-md border border-hairline-soft bg-white">
		<CodeEditor
			class="min-h-72"
			value={draft}
		...
		<div class="min-w-0 flex-1 basis-48">
			{#if validation.ok}
				<p class="text-xs text-ink-2">
					{isDirty ? '有効な式です。適用できます。' : '式は有効です。'}
				</p>
			{:else}
				<p class="text-xs break-words text-ink-2" role="alert">{validation.error}</p>
			{/if}
		</div>
		<div class="ml-auto flex shrink-0 items-center gap-1">
			<Button
				class="h-7 rounded px-2 text-xs font-semibold text-ink-3 disabled:cursor-default disabled:text-ink-4 disabled:hover:bg-transparent"
				disabled={!isDirty}
				onclick={revert}
			>
				元に戻す
			...
			<Button
				class="h-7 rounded bg-ink-2 px-2.5 text-xs font-semibold text-white hover:bg-ink-2 disabled:cursor-default disabled:bg-field disabled:text-ink-3"
				disabled={!isDirty || !validation.ok}
				title="JSON を適用 (⌘/Ctrl+Enter)"
				onclick={apply}
			>
				JSON を適用
			</Button>
